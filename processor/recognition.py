...
class REC_Processor(Processor):
    ...
    def train(self):
        ...
        self.io.print_log('Time consumption: {}s'.format(round(self.end_time - self.start_time,2)))
    ...
    def test(self, evaluation=True):
        ...
        for data, label in allsample:
            ...
            # print(f"{alld.shape}")
            with torch.no_grad():
                allmid, _ = self.model(alld) # 13670 % 32 = 6

            allclassfi = allmid.view(allmid.shape[0], allmid.shape[2])
            allclassfi = allclassfi.cpu().detach().numpy()
            clasf.append(allclassfi)

        # print(f"allmid shape = {allmid.shape}")
        # print(f"len_clasf = {len(clasf)}")
        totaltclas = np.empty(shape=(13670, feature_num)) # 403 % 32 = 19
        totaltlabel = np.array(alllabel)
        # print(f"{totaltlabel[1]}")
        bsinprocesser = 32
        count = 0
        # print(f"{len(clasf)}")
        for per_batch in clasf:
            if (count + bsinprocesser) > totaltclas.shape[0]:
                rest = totaltclas.shape[0] - count
                totaltclas[count:totaltclas.shape[0]] = per_batch[:rest,:]
                break
            else:
                totaltclas[count:count + bsinprocesser] = per_batch
            count += bsinprocesser
        #print(f"Handling finished, from row 0 to row {totaltclas.shape[0]} with shape{totaltclas.shape}.")
        ...
        #print(f"allmid shape {self.allmid.shape} and alllabel shape {self.alllabel.shape}")
        ...
        for data, label in loader:
            ...
            classfi = mid.view(mid.shape[0], mid.shape[2])
            classfi = classfi.cpu().detach().numpy()
            # print(f"classfi with shape {classfi.shape} added in tclasf")
        ...
        totalteclasf = np.empty(shape=(4562, feature_num))
        totaltelabel = np.array(alltelabel)
        tebsinprocesser = 32
        count = 0

        for per_batch in tclasf:
            if (count + bsinprocesser) > totalteclasf.shape[0]:
                trest = totalteclasf.shape[0] - count
                totalteclasf[count:totalteclasf.shape[0]] = per_batch[:trest,:]
                break
            else:
                totalteclasf[count:count + bsinprocesser] = per_batch
            count += tebsinprocesser
        # print(f"tclasf shape = {len(tclasf)}")
        ...
        if evaluation:
            ...
            # show top-k accuracy and pass the consequences
            self.consequences = [0 for self.i in range(6)]
            for k in self.arg.show_topk:
                self.consequences[k] = self.show_topk_knn(k, totalteclasf, totaltelabel)
                self.io.print_log('\tTop{}: {:.2f}%'.format(k, 100 * self.consequences[k]))
                # print(f"Successfully passing Top{k} to consequences[{k}]: {self.consequences[k]}")
